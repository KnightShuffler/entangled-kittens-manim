...
class Postulate1_Statement(Scene):
    def construct(self):
        ...
        statement = [
                        TextMobject(r'Every '),
                        TextMobject(r'isolated quantum system',color='yellow'),
                        TextMobject(r'has an associated'),
                        TextMobject(r"``Hilbert Space''",color='yellow'), 
                        TextMobject(r' called the'),
                        TextMobject(r'\textit{state space}',color='yellow'),
                        TextMobject(r' of that system. The system is completely'),
                        TextMobject(r'described by the '),
                        TextMobject(r'\textit{state vector,}',color='yellow'),
                        TextMobject(r' a unit vector in the state space.')
        ]
        ...
        statement[4].next_to(statement[0],direction=DOWN,aligned_edge=LEFT_SIDE)
        statement[5].next_to(statement[4],direction=0.8*RIGHT)
        statement[6].next_to(statement[5],direction=0.8*RIGHT)
        statement[7].next_to(statement[4],direction=DOWN,aligned_edge=LEFT_SIDE)
    ...
    def highlight_and_explain(self, base, definition):
        ...
        defn = TextMobject(definition).next_to(title,direction=DOWN)
